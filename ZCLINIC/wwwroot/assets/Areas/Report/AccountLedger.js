...
$('#LedgerPrint').click(function (e) {
    ...
    var Account = 'Form Account : ' + $("#FAccountCode").val() + '-' + $('#FAccountName option:selected').text()  +  
        ' Up to Account: ' + $("#TAccountCode").val() + '-' + $('#ToAccountName option:selected').text();
  ...
  /*  if ($('#ReportId').val() == 1) {*/
    ...
    var value = `/Report/Reportrdlc/AReport?rvalues.Name=7&rvalues.Value[0]=${$('#FromDate').val()}&rvalues.Value[1]=${$('#ToDate').val()}&rvalues.Value[2]=${$('#FAccountCode').val()}&rvalues.Value[3]=${$('#TAccountCode').val()}&rvalues.Value[4]=1&rvalues.Value[5]=${Account}&rvalues.Value[6]=${Date}`
