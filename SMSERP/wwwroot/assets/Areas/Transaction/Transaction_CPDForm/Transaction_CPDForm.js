...
var image = $('#ImagePath').val() == "" || $('#ImagePath').val() == null ? "" : "/assets/Photo/" + $('#ImagePath').val()
...
$('.file-input').change(function (e) {
    ...
    if (result === false) {
        ...
        $.ajax({
            url: "/Transaction/Transaction_CPDForm/UploadImage",
            type: 'POST',
            cache: false,
            contentType: false,
            processData: false,
...
if ($('#CPDID').val() > 0) {
  ...
  //  $('tbody.BankVoucherDetails tr').each(function () {
   ...
   //     let store = $(this).children().eq(0);
    ...
    document.getElementById('R_MDate').valueAsDate = new Date();
